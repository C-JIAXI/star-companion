...
| Legacy browser queue compatibility | Existing sessionStorage queue, read only after unlock | Same WebView compatibility reader | Old item IDs and existing draft references | Original copy retained until confirmed recovery/discard; only acknowledgement IDs are newly stored, never text or binary | No | Missing/expired references need explicit text-only recovery; never auto-send |
