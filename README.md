...
- In-chat AI Agent with local per-chat conversation history and follow-up tasks for scene summaries, next-step suggestions, reply drafts, memory/lore candidates, continuity checks, and character-consistency checks. Models explicitly configured with `tool_calling` can search and read visible messages, recall enabled long-term memories, and read the current character's accessible prompt and enabled embedded Lore through native OpenAI-compatible, Anthropic, or Gemini tool calls; other models continue with text-only assistance. Execution steps stream over WebSocket and can be resumed by run ID after reconnecting; a status endpoint preserves the terminal state after event replay expires. Each task has editable temperature and output length, subject to the selected model's limit. Reply drafts can be inserted individually, with append or replace choice when the composer has text. Draft and memory/lore candidates use validated JSON; memory creation, update, merge, disable, and embedded-lore creation or update use editable previews, backend confirmation, and idempotent receipts. Memory merges use existing revision history and can be undone as one operation. Lore confirmation checks the character version and private-card password boundary. Agent reference material is separated from its instructions, and candidate sources must be explicitly cited from messages or memories the Agent received or read through tools. See [the upgrade progress](docs/agent-upgrade-implementation.md).
- Four built-in Skills and imported `SKILL.md` or ZIP packages can be reviewed and enabled under Settings → Skills & MCP. Each chat can select its own Skills in Chat settings → Character tools. Imported Skills start disabled; replacing one requires confirmation. Enabled methods are listed briefly and their instructions or text references load on demand. Scripts and dependencies are never run. Skill packages remain on this device and are excluded from backup and sync.
- Remote MCP servers can be added, checked, edited, enabled, and removed under Settings → Skills & MCP; discovered tools are enabled per server. The client discovers tools over Streamable HTTP with 2026-07-28 / 2025-11-25 negotiation, encrypts optional Bearer tokens, requires HTTPS for public endpoints, and invalidates tool activation when definitions change. External calls show the destination, tool, and arguments for approval; a user can grant a specific read tool for the current session. Ordinary role chat has a per-chat tool switch, uses the chat model, and exposes only selected built-in, Skill, and MCP tools. Its tool steps appear separately from the final character reply.
- Built-in web tools let the story assistant and opted-in character chats search the public web and read public HTTPS pages. Configure a Brave Search API key under Settings → Web, Skills & MCP to enable search; page reading needs no key. Every call shows its arguments and requires approval. The key is encrypted locally and excluded from backup and sync. Brave search usage is billed by Brave separately from the app's model budget. Web requests block private addresses, redirects, unsupported content, and oversized responses; returned text is untrusted reference material.
- Per-module model preferences for chat, AI Agent, memory maintenance, memory embeddings, user profile summaries, voice transcription, text-to-speech, and image generation, with capability-filtered model choices and server-side validation. Temperature, maximum output tokens, and Top P are edited per model from the chat model picker; existing models without explicit values retain the previous defaults.
