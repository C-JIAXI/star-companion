...
- Chat-scoped long-term memory with automatic maintenance, hybrid semantic-vector and keyword retrieval, source- and dimension-validated index status, keyword fallback while an index is stale or unavailable, and a dedicated index rebuild action with ready/stale/failed progress. Reusable local persona presets, user profile summaries, preflighted local backup import/export, bounded recovery points, and conflict-safe manual LAN sync are also included. A persona preset can retain the visible user name and an optional local avatar for that chat; a stable placeholder is generated when no avatar is uploaded, while model-facing identity remains explicit in the preset prompt fields.
