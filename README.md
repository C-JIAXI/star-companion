...
- Four built-in Skills and imported `SKILL.md` or ZIP packages can be reviewed and enabled for the story assistant or an individual chat. Imported Skills start disabled; replacing one requires confirmation. Enabled methods are listed briefly and their instructions or text references load on demand. Scripts and dependencies are never run. Skill packages remain on this device and are excluded from backup and sync.
- Remote MCP connections can be saved and checked locally. The client discovers tools over Streamable HTTP with 2026-07-28 / 2025-11-25 negotiation, encrypts optional Bearer tokens, requires HTTPS for public endpoints, and invalidates tool activation when definitions change. External calls show the destination, tool, and arguments for approval; a user can grant a specific read tool for the current session. Ordinary role chat has a per-chat tool switch, uses the chat model, and exposes only selected built-in, Skill, and MCP tools. Its tool steps appear separately from the final character reply.
