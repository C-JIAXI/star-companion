...
test("chat agent panel inserts reply drafts and confirms memory candidates before saving", async ({
  ...
  try {
    ...
    const importedSkillRow = skillPanel.getByTestId(`skill-row-${skillName}`);
    ...
    await mcpPanel.getByRole("button", { name: /添加服务器|Add server/ }).click();
    ...
    const mcpRow = mcpPanel.locator('[data-testid^="mcp-connection-"]').filter({ hasText: `MCP ${suffix}` });
    await expect(mcpRow.getByText(/密钥已配置|Token configured/)).toBeVisible();
    await mcpRow.getByRole("button", { name: /编辑|Edit/ }).click();
    await mcpPanel.getByLabel(/连接名称|Connection name/).fill(`Edited MCP ${suffix}`);
    await mcpPanel.getByRole("button", { name: /保存修改|Save changes/ }).click();
    const editedMcpRow = mcpPanel.locator('[data-testid^="mcp-connection-"]').filter({ hasText: `Edited MCP ${suffix}` });
    await expect(editedMcpRow.getByText(/密钥已配置|Token configured/)).toBeVisible();
    await editedMcpRow.getByRole("button", { name: /删除|Delete/ }).click();
    await page.getByRole("dialog").getByRole("button", { name: /删除|Delete/ }).click();
