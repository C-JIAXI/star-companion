...
test("quick commands expand to show choices and collapse again", async ({ page, request }) => {
  ...
  try {
    ...
    expect(await list.evaluate((element) => getComputedStyle(element).animationName)).toBe("quick-replies-enter");
    ...
    await expect(page.locator("[data-chat-quick-reply]")).toHaveCount(12);
    ...
    await toggle.click();
    expect(await list.evaluate((element) => getComputedStyle(element).animationName)).toBe("none");
