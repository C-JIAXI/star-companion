...
const codes = (draft: ReturnType<typeof base>) => checkCharacterQuality(draft).issues.map((item) => item.code);
...
describe("character quality checker", () => {
  ...
  for (const [code, draft] of cases) it(`emits stable code ${code}`, () => assert.ok(codes(draft).includes(code)));
