...
test("the bundled Prisma SQL catalog initializes a fresh desktop database deterministically", () => {
  ...
  assert.ok(report.appliedMigrations.includes("20260923020000_agent_memory_sources"));
  ...
  for (const table of ["ChatDraft", "DraftHandoff", "AgentSession", "AgentEntry", "SkillPackage", "McpConnection"]) {
    assert.equal(db.prepare("SELECT COUNT(*) AS count FROM sqlite_master WHERE type='table' AND name=?").get(table).count, 1);
  }
  assert.equal(db.prepare("SELECT COUNT(*) AS count FROM pragma_table_info('AgentEntry') WHERE name='generation'").get().count, 1);
